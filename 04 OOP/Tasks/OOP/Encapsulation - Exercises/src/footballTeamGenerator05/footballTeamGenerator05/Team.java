...
import shoppingSpree03.Validator;
...
public class Team {
    ...
    public void removePlayer(String namePlayer) {
        if (!this.players.removeIf(p -> p.getName().equals(namePlayer))) {
            throw new IllegalArgumentException("Player " + namePlayer + " is not in " + this.getName() + " team.");
        }
    }

    public double getRating() {
        return this.players.stream().mapToDouble(Player::overallSkillLevel).average().orElse(0) / 5;
    }
